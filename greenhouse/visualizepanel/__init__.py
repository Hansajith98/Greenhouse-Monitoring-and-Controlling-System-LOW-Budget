...
def stream_handler(message):
    if "Humidity" in message["data"]:
        ...
        if message['data']['Humidity'] < crop_min_max[ crop_device_data[message["data"]["DeviceId"]][0] ]['min']['Humidity']:
            database.child("Controller").child(crop_device_data[message["data"]["DeviceId"]][1]).child("Sprayer").set(1)
        else:
            database.child("Controller").child(crop_device_data[message["data"]["DeviceId"]][1]).child("Sprayer").set(0)
